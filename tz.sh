...
Download_Server_Status_server(){
	cd "/usr/local"
	wget -N --no-check-certificate "https://github.com/FeulerLoup/ServerStatus/archive/master.zip"
...
Service_Server_Status_server(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/FeulerLoup/scripts/master/attach/server_status_server_centos" -O /etc/init.d/status-server; then
		...
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/FeulerLoup/scripts/master/attach/server_status_server_debian" -O /etc/init.d/status-server; then
			echo -e "${Error} ServerStatus 服务端服务管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/status-server
...
Service_Server_Status_client(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/FeulerLoup/scripts/master/attach/server_status_client_centos" -O /etc/init.d/status-client; then
		...
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/FeulerLoup/scripts/master/attach/server_status_client_debian" -O /etc/init.d/status-client; then
			echo -e "${Error} ServerStatus 客户端服务管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/status-client
...
默认为本机IP为域名，例如输入: feuler.cn，如果要使用本机IP，请留空直接回车"
...
Set_password(){
	...
	stty erase '^H' && read -p "(默认: feuler.cn):" password_s
	[[ -z "$password_s" ]] && password_s="feuler.cn"
	echo && echo "	================================================"
